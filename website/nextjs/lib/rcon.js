...
class RconClient {
  ...
  execute(command) {
    return new Promise((resolve, reject) => {
      ...
      socket.on('connect', () => this._send(socket, 3, this.password, 1));
...
export async function applyPerks(purchaseId, minecraftUsername, commands) {
...
export async function getServerStatus() {
  try {
    ...
    return { online: true, message: result };
  } catch {
    return { online: false, message: 'Server offline or RCON unavailable' };
  }
}

export function startPerkWorker() {
  flushQueue();
